...
const isWriteQuery = (cypher: string): boolean => {
  const readProcedureSafeCypher = cypher.replace(
    /\bCALL\s+db\.index\.fulltext\.queryNodes\b/gi,
    'CALL gitnexus_read_fulltext_queryNodes',
  );
  return /\b(CREATE|MERGE|SET|DELETE|REMOVE|DROP|LOAD\s+CSV|CALL\s+db\.create|CALL\s+db\.index|CALL\s+apoc\.create|CALL\s+apoc\.periodic)\b/i.test(
    readProcedureSafeCypher,
  );
};
