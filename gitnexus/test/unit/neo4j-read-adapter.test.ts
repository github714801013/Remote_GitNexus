...
describe('Neo4j read adapter', () => {
  ...
  it('allows Neo4j fulltext index read procedures', async () => {
    txRun.mockResolvedValueOnce({
      records: [record({ id: 'File:a', filePath: 'src/a.ts', score: 1.2 })],
    });
    const { executeReadCypher } = await import('../../src/core/neo4j/read-adapter.js');

    const rows = await executeReadCypher(
      "CALL db.index.fulltext.queryNodes('file_fts', $query) YIELD node, score RETURN node.id AS id, node.filePath AS filePath, score",
      { query: 'handler' },
    );

    expect(txRun).toHaveBeenCalledWith(expect.stringContaining('db.index.fulltext.queryNodes'), {
      query: 'handler',
    });
    expect(rows).toEqual([{ id: 'File:a', filePath: 'src/a.ts', score: 1.2 }]);
  });
